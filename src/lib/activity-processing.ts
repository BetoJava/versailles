...
`;
    ...
    const batchPromises = batches.map(async (batch): Promise<ScoredActivity[]> => {
        ...
        try {
            ...
            // Extraire le JSON de la réponse (nouveau format avec batch)
            ...
            let scores: LLMScoreResponse[] = [];
            try {
                // Nettoyer le JSON avant de le parser (enlever les virgules trailing)
                const cleanedJson = jsonMatch[1].replace(/,(\s*[}\]])/g, '$1');
                const responseData = JSON.parse(cleanedJson);
                scores = responseData.batch || [];
                // console.log("Scores extraits du batch:", scores);
            } catch (error) {
                console.error("Erreur lors de l'extraction du JSON:", error, "JSON:", jsonMatch[1]);
                return batch.map(activity => ({ ...activity, score: 3 }));
            }

            // Mapper les scores aux activités
    ...
    const filteredActivities = scoredActivities.filter((activity): activity is ScoredActivity =>
        activity !== undefined && activity.score !== undefined && activity.score > 0
    );
